<div class="px-4 py-5 bg-white sm:p-6">
    ...
    <select  id="sector-dd" style="width:400px"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option value="{{$sector->id}}">{{$sector->industrial_sector}}</option>
...
<div class="px-4 py-5 bg-white sm:p-6">
    ...
    <select id="subsector-dd" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        <option value="{{$sub_sector->id}}">{{$sub_sector->industrial_sub_sector}}</option>
    </select>
</div>
...
<div class="px-4 py-5 bg-white sm:p-6">
    ...
    <select id="definition-dd" name="sektor_industri" class="break-normal normal-case bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        <option value="{{$definition->industrial_code}}">{{$definition->industrial_code}} - {{$definition->industrial_definition}}</option>
    </select>
...
<div class="px-4 py-5 bg-white sm:p-6">
    <div class="flex">
        <div class="flex-1">
            ...
            <input value="{{$lpg->warna_lpg}}" id="warna_lpg_input" style="width:100px" type="text" disabled class="{{$lpg->color}} form-input rounded-md shadow-sm mt-1 block w-full"/>
        </div>
        <div class="flex-1">
            <label class="block font-medium text-sm text-gray-700">%MS</label>
            <input value="{{$lpg->persen_MS}}"  id="persen_MS_input" style="width:100px" type="text" disabled class="bg-gray-50 form-input rounded-md shadow-sm mt-1 block w-full"/>
        </div>
        <div class="flex-1">
            <label class="block font-medium text-sm text-gray-700">%DPK</label>
            <input  value="{{$lpg->persen_DPK}}" id="persen_DPK_input" style="width:100px" type="text" disabled class="bg-gray-50 form-input rounded-md shadow-sm mt-1 block w-full"/>
        </div>
        <div class="flex-1">
            <label class="block font-medium text-sm text-gray-700">%NPL</label>
            <input  value="{{$lpg->persen_NPL}}" id="persen_NPL_input" style="width:100px" type="text" disabled class="bg-gray-50 form-input rounded-md shadow-sm mt-1 block w-full"/>
        </div>
    </div>
</div>
    ...
    $('#sector-dd').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                $('#subsector-dd').html('<option value="{{$sub_sector->id}}">{{$sub_sector->industrial_sub_sector}}</option>');
    ...
    $('#subsector-dd').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                $('#definition-dd').html('<option value="{{$definition->industrial_code}}">{{$definition->industrial_code}} - {{$definition->industrial_definition}}</option>');
